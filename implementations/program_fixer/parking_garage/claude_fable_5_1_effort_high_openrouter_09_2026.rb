...
class ParkingGarage
  ...
  def find_car(license_plate_no)
    ...
    @parking_spots.each do |spot, cars|
      ...
      return { spot: spot.to_s.sub('_spot', ''), car: car } if car
    end
    nil
  end
  ...
  # A medium car needs a medium or large spot. Free one by relocating a
  # small car parked in a medium/large spot into a free small spot.
  ...
  # A large car needs a large spot. Free one by relocating a medium car
  # (to a medium spot) or a small car (to a small or medium spot).
