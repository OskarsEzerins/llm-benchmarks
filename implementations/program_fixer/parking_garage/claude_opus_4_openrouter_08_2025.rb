...
class ParkingGarage
  ...
  def shuffle_medium(kar)
    if @small > 0
      small_car = @parking_spots[:small_spot].find { |c| c[:size] == 'small' }
      if small_car
        @parking_spots[:small_spot].delete(small_car)
        temp_spot = @parking_spots[:medium_spot].any? ? :medium_spot : :large_spot
        @parking_spots[temp_spot] << small_car
        @parking_spots[:medium_spot] << kar
        @small -= 1
  ...
  def exit_status(plate = nil)
    plate ? "car with license plate no. #{plate} exited" : nil
  end
end
