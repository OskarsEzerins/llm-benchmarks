...
class ParkingGarage
  ...
  def admit_car(license_plate_no, car_size)
    ...
    return "No space available" if car_size.nil? || car_size.to_s.strip.empty?
  ...
  def shuffle_medium(kar)
    ...
    if small_in_medium && @small > 0
      ...
      @small -= 1
      @parking_spots[:medium_spot] << kar
      @medium += 0 # medium count stays same (one out, one in)
      parking_status(kar, 'medium')
    else
      parking_status
    end
...
class ParkingGarageManager
  ...
  def exit_car(plate)
    ...
    return { success: false, message: "No ticket found for #{plate}" } unless ticket
  ...
  def garage_status
    occupied = @garage.parking_spots[:small_spot].length +
