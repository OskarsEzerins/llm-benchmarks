...
class ParkingGarage
  ...
  def shuffle_medium(car)
    # Try to move a small car out of a medium spot into an available small spot
    ...
    # Try to move a small car out of a large spot into an available small spot
  ...
  def shuffle_large(car)
    # Try to move a medium car out of a large spot into an available medium spot
    ...
    # Try to move a small car out of a large spot into an available small/medium spot
    small_in_large = @parking_spots[:large_spot].find { |c| c[:size] == 'small' }
    if small_in_large
      if @small > 0
        @parking_spots[:large_spot].delete(small_in_large)
  ...
  def exit_status(plate = nil)
    if plate
      ...
      'No car found with this license plate'
    end
  end
end
...
class ParkingGarageManager
  ...
  def admit_car(license_plate, car_size)
    ...
    if result.to_s.include?('parked')
      plate_key = license_plate.to_s.strip
      ticket = ParkingTicket.new(plate_key, car_size)
  ...
  def find_ticket(license_plate)
    @active_tickets.fetch(license_plate.to_s.strip, nil)
  end
end