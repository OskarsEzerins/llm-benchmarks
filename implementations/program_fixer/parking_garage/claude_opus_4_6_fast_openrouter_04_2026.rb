...
class ParkingGarage
  ...
  def shuffle_medium(kar)
    # Find a small car in a medium spot that can be moved to a small spot
    ...
    # Find a small car in a large spot that can be moved to a small spot
  ...
  def shuffle_large(kar)
    # Find a medium car in a large spot that can be moved to a medium spot
...
class ParkingFeeCalculator
  ...
  def calculate_fee(car_size, duration_hours)
    ...
    total = hours * rate
    max = MAX_FEE[car_size] || total
    [total, max].min.to_f
  end
end

class ParkingGarageManager
  def initialize(small_spots, medium_spots = nil, large_spots = nil)
    if small_spots.is_a?(Hash)
      s = small_spots[:small_spots] || small_spots[:small] || 0
      m = small_spots[:medium_spots] || small_spots[:medium] || 0
      l = small_spots[:large_spots] || small_spots[:large] || 0
