...
class ParkingGarage
  ...
  def shuffle_medium(kar)
    ...
    if @parking_spots[:medium_spot].include?(victim) && @small > 0
      @parking_spots[:medium_spot].delete(victim)
      @parking_spots[:small_spot] << victim
      @small -= 1
      @parking_spots[:medium_spot] << kar
      parking_status(kar, 'medium')
    elsif @parking_spots[:large_spot].include?(victim) && @medium > 0
...
class ParkingGarageManager
  ...
  def admit_car(plate, size)
    ...
    return { success: false, message: 'Invalid car size' } unless %w[small medium large].include?(normalized_size)

    plate   = plate.to_s
    verdict = @garage.admit_car(plate, normalized_size)

    if verdict.to_s.include?('parked at')
