class Calendar
  ...
  DAYS_IN_MONTHS = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31].freeze
  SAKAMOTO_OFFSETS = [0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4].freeze

  def initialize(year)
    @year = valid_year?(year) ? year : Time.now.year
  end
  ...
  def day_of_week(month, day)
    ...
    y = year
    y -= 1 if month < 3
    (y + y / 4 - y / 100 + y / 400 + SAKAMOTO_OFFSETS[month - 1] + day) % 7
  end
