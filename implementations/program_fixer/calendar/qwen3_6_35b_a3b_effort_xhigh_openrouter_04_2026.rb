...
class Calendar
  ...
  def initialize(year)
    @year = year.is_a?(Integer) && year > 0 ? year : 1900
  end
  ...
  def days_in_month(month)
    ...
    days[month] += 1 if month == 2 && is_leap_year?
    days[month]
  end

  def day_of_week(month, day)
    return nil unless month.is_a?(Integer) && day.is_a?(Integer) && month.between?(1, 12)
    begin
  ...
  def get_weekdays_in_month(month)
    ...
    weekdays_names = %w[Sunday Monday Tuesday Wednesday Thursday Friday Saturday]
    (1..days_in_month(month)).map do |d|
      weekdays_names[day_of_week(month, d)]
    end
  end

  def count_occurrences_of_day(month, target_day)
    return 0 unless month.is_a?(Integer) && target_day.is_a?(Integer) && target_day.between?(0, 6)
    count = 0
