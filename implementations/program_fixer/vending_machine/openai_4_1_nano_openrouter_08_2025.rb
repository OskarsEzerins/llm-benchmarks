class VendingMachine
  ...
  def select_item(label)
    return 'Item not found' if label.nil? || label.strip.empty?
  ...
  def restock(label, qty)
    return unless label.is_a?(String) && qty.is_a?(Integer) && qty >= 0
  ...
  # Optional: add attr_reader for balance if needed
  def balance
    @balance
  end
end