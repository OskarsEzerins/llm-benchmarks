class VendingMachine
  def initialize(items = nil)
    ...
    if items.is_a?(Array)
      items.each do |item|
        if item.is_a?(Hash) && item[:name].is_a?(String) && !item[:name].empty? &&
           item[:price].is_a?(Numeric) && item[:price] > 0 &&
  ...
  def select_item(name)
    return "Item not found" if !name.is_a?(String) || name.strip.empty?
  ...
  def check_stock(name)
    return 0 if !name.is_a?(String) || name.strip.empty?
    item = @inventory.find { |i| i[:name] == name }
