class VendingMachine
  def initialize(items = nil)
    @inventory = items && items.is_a?(Array) ? items.dup : []
    @inventory.each do |item|
      next unless item.is_a?(Hash) && item.key?(:name) && item.key?(:price) && item.key?(:quantity)
      item[:quantity] = item[:quantity].to_i
      item[:price] = item[:price].to_f
    end
    @inventory.reject! { |item| !item.is_a?(Hash) || !item.key?(:name) || !item.key?(:price) || !item.key?(:quantity) }
    @balance = 0.0
  end
  ...
  def restock(name, quantity)
    return if name.nil? || name.to_s.strip.empty? || !quantity.is_a?(Numeric) || quantity <= 0
