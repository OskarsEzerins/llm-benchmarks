...
│   │   ├── model-name-utils.ts      # Model name normalization
│   │   └── utils.ts      # cn() utility
...
├── public/data/          # Synced JSON data files
│   ├── *.json            # Benchmark results
│   └── implementations.json  # Implementation manifest (metadata only)
...
- **Client-side**: Implementation source code fetched from GitHub raw content on-demand
- **GitHub Raw Base**: `https://raw.githubusercontent.com/OskarsEzerins/llm-benchmarks/main/implementations`
...
- Use `normalizeModelName()` and `getModelFamily()` for display names
